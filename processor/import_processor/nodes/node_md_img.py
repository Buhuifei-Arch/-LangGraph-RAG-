...
class NodeMDImg(BaseNode):
    ...
    def _step_4_upload_and_replace(self,doc_stem: str,target_images: List[Tuple[str, str, Tuple[str, str]]],summaries: Dict[str, str],md_content: str) -> str:
        ...
        # 构造上传目录，去除文件名中的空格
        minio_img_dir = import_config.get_config().minio_img_dir
    ...
    def _step_3_generate_summaries(self, doc_stem: str,target_images: List[Tuple[str, str, Tuple[str, str]]]) -> Dict[str, str]:
        ...
        # 2、循环处理图片
        for img_file,image_path,content in target_images:
            # 2.1、速率限制
            self._apply_api_rate_limit(request_deque,max_requests=10)
            # 2.2、调用大模型生成图片摘要
            summaries[img_file] = self._summarize_image(image_path, root_folder=doc_stem, image_content=content)
    ...
    def _apply_api_rate_limit(self,request_times:Deque[float],max_requests:int,window_seconds:int = 10)->None:
        """
        ...
        # 计算需要等待的时长（窗口总时长 - 最早请求已存在的时长）
            ...
            if sleep_duration>0:
                self.logger.info(f"触发API速率限制，窗口{window_seconds}秒内最多{max_requests}次，需等待：{sleep_duration:.2f} 秒")
    ...
    def _summarize_image(self, image_path: str, root_folder: str, image_content: Tuple[str, str]) -> str:
        ...
        try:
            ...
            return response.content.strip().replace("/n", "")
    ...
    def _clean_minio_directory(self,minio_client: Minio, prefix: str) -> None:
        ...
        """
        try:
            objects_to_delete = minio_client.list_objects(import_config.get_config().minio_bucket, prefix= prefix, recursive = True)
            # 构造删除列表
            delete_list = [DeleteObject(import_config.get_config().minio_bucket) for obj in objects_to_delete]
            if delete_list:
                errors = minio_client.remove_objects(import_config.get_config().minio_bucket, delete_list)
                for error in errors:
    ...
    def _upload_images_batch(self, minio_client: Minio, upload_dir: str, target_images: List[Tuple]) -> Dict[str, str]:
        ...
        for img_file ,img_path,_ in target_images:
            object_name = f"{upload_dir}/{img_path}"
       ...
       """
        ...
        try:
            ...
            minio_client.fput_object(
                bucket_name=import_config.get_config().minio_bucket, # MinIO存储桶名（从配置读取）
            ...
            # 构造MinIO基础访问URL
            base_url = f"{import_config.get_config().minio_endpoint}/{import_config.get_config().minio_bucket}"
...
if __name__ == "__main__":
    ...
    md_path = r"D:\zk_data\output\H3C ER2100企业级路由器 用户手册-6W104-整本手册\H3C ER2100企业级路由器 用户手册-6W104-整本手册.md"
