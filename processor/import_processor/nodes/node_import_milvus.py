...
class NodeImportMilvus(BaseNode):
    ...
    def _step_4_insert_data(self, client, chunks_json_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        # 2. 执行批量插入
        insert_result = client.insert(collection=milvus_config.collection, data=data_to_insert)
        ...
        if inserted_ids:
            for ids,item in enumerate(inserted_ids):
                item["chunk_id"] = str(ids)
...
if __name__ == "__main__":
    ...
    json_path = r"D:\zk_data\output\H3C ER2100企业级路由器 用户手册-6W104-整本手册\state_vector.json"
