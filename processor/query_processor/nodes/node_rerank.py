...
class NodeRerank(NodeBase):
    ...
    def _step_1_merge_multi_source_docs(self, state: QueryGraphState) -> List[Dict[str, Any]]:
        ...
        final_docs = []
        # 1. 获取本地 RRF 的文档
        for rrf_doc in state['rrf_chunks']:
            formatted_rrf_doc = {
                "content": rrf_doc["content"],
                "title": rrf_doc["title"],
                "chunk_id": rrf_doc["chunk_id"],
                "url":None,
                "source":"local"
            }
            final_docs.append(formatted_rrf_doc)

        # 2. 获取 web 远程的文档
        for wbe_doc in state['web_search_docs']:
            formatted_wbe_doc = {
                "content": wbe_doc["snippet"],
                "title": wbe_doc["title"],
                "chunk_id": None,
                "url":wbe_doc["url"],
                "source":"web"
            }
            final_docs.append(formatted_wbe_doc)
        return final_docs
