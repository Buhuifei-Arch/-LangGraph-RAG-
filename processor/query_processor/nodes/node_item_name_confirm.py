...
from sympy.polys.matrices import dense

from config import lm_config, embedding_config
...
class NodeItemNameConfirm(NodeBase):
    ...
    def _step_5_vectorize_and_query(self, item_names) -> List[Dict]:
        ...
        for i in range(len(item_names)):
            try:
                ...
                spares_vector = embeddings.get("spare")[i]
                ...
                if search_res and len(search_res) > 0:
                    ...
                    for hit in search_res[0]:
                        ...
                        matches.append(
                            {
                                "item_name": hit.get("entity",{}).get("name"),
    ...
    def _step_6_align_item_names(self, query_results) -> dict:
        ...
        for res in query_results:
            ...
            # 筛选高置信度匹配结果：评分>0.85
            high = [m for m in matches if m["score"] > 0.85]
            # 筛选中置信度匹配结果：评分≥0.6（仅高置信度为空时生效）
            mid = [m for m in matches if m["score"] >= 0.6]

            if len(high) >= 1:
                for hit in high:
                    confirmed_item_names.append(hit.get("item_name"))
                continue
        ...
        return {
                "confirmed_item_names": list(set(confirmed_item_names)),  # 去重后的确认商品名，无则空列表
                "options": list(set(options))          # 去重后的候选商品名，无则空列表
                }
